...
window.exportarTerritorioTXT = async function () {
  ...
  const entradas = datos.filter(d => d.numero === num);
...
window.restaurarDesdeArchivo = async function () {
  ...
  lector.onload = async (e) => {
    try {
      ...
      const snap = await getDocs(collection(db, "territorios"));
      // usar batch para acelerar (si hay muchos)
      const batchDeletes = snap.docs.map(d => deleteDoc(doc(db, "territorios", d.id)));
      await Promise.all(batchDeletes);

      // 2) añadir los del respaldo
      const adds = parsed.map(item => addDoc(collection(db, "territorios"), item));
...
document.getElementById("btnTelegram").addEventListener("click", async () => {
  ...
  if (!lastResults || lastResults.length === 0) {
    ...
    await cargarDatos();
    lastResults = datos.filter(d => d.numero == filtro);
    if (lastResults.length === 0) return alert("No hay resultados para enviar");
  }

  const mensaje = lastResults.map((e, i) =>
